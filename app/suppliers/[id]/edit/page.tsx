...
export default function EditSupplierPage({ params }: { params: { id: string } }) {
    ...
    const fetchSupplier = async () => {
      try {
        ...
        form.reset({
          ...
          website: formatWebsiteForDisplay(supplierData.website) || "",
        ...
        setAuxiliaryEmails(Array.isArray((data as any).auxiliaryEmails) ? (data as any).auxiliaryEmails : [])
      } catch (err) {
  ...
  // Prepare website field for display
  ...
  const onSubmit = async (data: SupplierFormValues) => {
    ...
    try {
      ...
      // Submit to API
      await updateSupplier(params.id, supplierData)
  ...
  const handleUpdateAuxEmail = async (emailId: string, field: 'name' | 'phone', value: string) => {
    try {
      ...
      // Send update to server - we'll need to add a PATCH endpoint for this
    ...
    return (
      <div className="container mx-auto py-8 max-w-5xl">
        <div className="bg-red-900/20 p-6 rounded-md text-center">
          ...
          <h3 className="text-lg font-medium text-red-500 mb-2">{loadError}</h3>
          <Button
            variant="outline"
            size="sm"
        ...
        <CardContent>
          {submitSuccess ? (
            <div className="bg-green-50 p-6 rounded-lg text-center">
              ...
              <h3 className="text-lg font-medium text-green-800">Supplier Updated Successfully</h3>
              <p className="text-green-600 mt-2">Redirecting to supplier details...</p>
            </div>
          ) : (
            <Form {...form}>
