...
export function SupplierResponseTabs({ 
  ...
  return (
    <Tabs defaultValue={sortedThreads[0]?.supplierId} className="w-full">
      <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${sortedThreads.length}, minmax(0, 1fr))` }}>
        {sortedThreads.map(thread => (
          <TabsTrigger key={thread.supplierId} value={thread.supplierId} className="relative">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  {onRefreshPrices && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        onRefreshPrices(thread.supplier.id);
                      }}
              ...
              {quoteItems && quoteItems.length > 0 && (() => {
                ...
                const supplierItems = quoteItems.filter((item: any) => 
                  item.supplierId === thread.supplier.id || (!item.supplierId && thread.isPrimary)
                );
                
                console.log('[SupplierTab] Quote items for supplier:', {
                  supplierId: thread.supplier.id,
                  supplierName: thread.supplier.name,
                  totalItems: quoteItems.length,
                  filteredItems: supplierItems.length,
                  itemDetails: supplierItems.map((item: any) => ({
                    id: item.id,
                ...
                return supplierItems.length > 0 ? (
                  <>
                    ...
                    <div>
                      <h4 className="font-semibold text-sm mb-3">Quote Items</h4>
                      <Table>
                        <TableHeader>
                          <TableRow>
              ...
              <div className="space-y-3">
                <h4 className="font-semibold text-sm">Communication History</h4>
